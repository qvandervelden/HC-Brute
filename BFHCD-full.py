...
N = 3 #The number of particles

#The identity
I = np.array([[1,0,0],[0,1,0],[0,0,1]])

#The spin-operators
Sx = h*(1/np.sqrt(2))*np.array([[0,1,0],[-1,0,1],[0,-1,0]])
Sy = h*(1/np.sqrt(2)*1j)*np.array([[0,1,0],[1,0,1],[0,1,0]])
Sz = h*np.array([[1,0,0],[0,0,0],[0,0,-1]])

#Computation of the tensor products of the spin-operators
S = np.empty(shape=(0,3,3**N,3**N))
for i in range(0,N):
    Sx_i = Sx
    Sy_i = Sy
    Sz_i = Sz
